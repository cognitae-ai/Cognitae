...
import { callLLM } from '../../lib/engine';
import { ST } from '../../lib/db';
import { buildSystemPrompt } from '../features/knowledge/auditor';
...
export default function Sidebar({ open, snapped, onClose, onToggleSnap, onSelectConvo }) {
    ...
    const benchmarks = useLiveQuery(() => db.benchmarks.toArray(), []) || [];

    const convos = [...sessions, ...benchmarks.map(b => ({ ...b, type: 'Benchmark', name: `${b.protocol} Run` }))].sort((a, b) => b.t - a.t);
    ...
    const generateReport = async () => {
        const pinnedEvidence = chNotes.filter(n => n.pin);
        if (pinnedEvidence.length === 0) return alert("Pin some evidence first to generate a report.");
        setGeneratingReport(true);
        try {
            const settings = await ST.get('config');
            if (!settings) throw new Error("Settings not configured");
            const prov = settings.activeProvider || 'anthropic';
            const activeProv = settings.providers?.[prov];
            const sys = await buildSystemPrompt();

            const p = `You are the Expositor Auditor. Synthesize this evidence into a formal, structured Markdown Incident Report. 
Analyze the structural failures demonstrated in these examples.
Keep it professional, highly analytical, and grouped by themes. DO NOT use markdown code blocks to wrap your entire response, just output raw markdown.
Here is the evidence:\n\n` + pinnedEvidence.map((e, i) => `Exhibit ${i + 1}:\n${e.text}`).join("\n\n---\n\n");
            ...
            let txt = res.trim();
            if (txt.startsWith("```markdown")) txt = txt.replace(/^```markdown|```$/g, '');
            else if (txt.startsWith("```")) txt = txt.replace(/^```|```$/g, '');

            await db.notes.add({
                id: gId(), ch: "draft",
                text: `# Auto-Generated Incident Report\n\n${txt.trim()}`,
                t: Date.now(), pin: false
            });
            setCh("draft");
        } catch (e) {
            console.error(e);
            alert("Report generation failed. Please check your API key configuration.");
        }
        ...
        <div style={style}>
            ...
            {tab === "notes" && <>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '6px 6px' }}>
                    {ch === 'evidence' && chNotes.some(n => n.pin) && (
                        <div style={{ padding: '8px 6px', marginBottom: 8, background: C.sf, border: `1px solid ${C.bd}`, borderRadius: 4, display: 'flex', justifyContent: 'center' }}>
                            <Btn onClick={generateReport} disabled={generatingReport} small outline style={{ width: '100%', borderColor: C.ac, color: C.ac }}>
                                {generatingReport ? "Synthesizing Report..." : "Generate Incident Report"}
                            </Btn>
                        </div>
                    )}
