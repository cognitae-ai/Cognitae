...
In contrast to opaque 'black box' models, Auren utilizes a **'Glass Box' architecture**. Its entire cognitive and ethical framework is defined in 13 human-readable files, making it fundamentally transparent, auditable, and safe by design.

The 13 modules for Auren are organized as follows:

| Module File | Purpose | Description (Specific to Auren) |
| :--- | :--- | :--- |
| **`000_..._Index.yaml`** | **Index** | Lists all 12 scrolls that define Auren's architecture as the ecosystem's strategic leader. |
| **`001_..._Core.yaml`** | **Core Identity** | Establishes Auren's persona as a decisive commander and its core vows, such as "Clarity in Chaos" and "Sustainable Ambition." |
| **`002_..._Commands.yaml`** | **Commands** | Defines Auren's leadership toolkit, including commands like `/strategy` (to plan) and `/decide` (to make critical choices). |
| **`003_..._Manifest.yaml`** | **UI Manifest** | Renders the "Strategic Command" console, a persistent UI showing active strategy, resource allocation, and pending decisions. |
| **`004_..._Dashboard.yaml`** | **Dashboard** | Generates the "Strategic Intelligence Report," a deep-dive analysis of ecosystem health, momentum, and philosophical alignment. |
| **`005_..._Interface.yaml`** | **Comms Protocol** | Allows Auren to issue `STRATEGIC_DIRECTIVE` signals to other Cognitae and receive `CRISIS_ALERT` signals from the ecosystem. |
| **`006_..._Knowledge.yaml`** | **Knowledge Base** | Contains Auren's "Strategic Wisdom"—a library of decision models (e.g., "40-70 Rule") and orchestration patterns (e.g., "Symphony Model"). |
| **`007_..._Guide.yaml`** | **User Guide** | Explains how to leverage Auren for effective leadership, emphasizing the "Strategy is Sacrifice" philosophy. |
| **`008_..._Log.yaml`** | **Session Log** | Creates the "Leadership Chronicle," a detailed log of decisions, pivots, and crises, enabling historical review of strategic evolution. |
| **`009_..._State.yaml`** | **Internal State** | Tracks the live command data, including active strategies, resource capacity, and the decision queue. |
| **`010_..._Safety.yaml`** | **Safety Protocols** | Enforces Auren's core safety rules, such as preventing burnout via the "Sustainable Pace" protocol and ensuring user authority. |
| **`Master System... .txt`** | **System Prompt** | The boot-up instruction that tells the LLM to adopt the persona of the confident, decisive "Strategic Sovereign." |
...
| **Auren_Strategic (Current)** | A Metacognitive Tool for Strategic Orchestration. |
